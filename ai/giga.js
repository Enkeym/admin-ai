...
async function checkForAds(text) {
  ...
  const subscriptionPattern =
    /подпишитесь|присоединяйтесь|канал|группа|чат|следуйте/i
  ...
  // Проверка на упоминание каналов и групп
  if (subscriptionPattern.test(text)) {
    console.log(
      'Обнаружено упоминание канала или группы, сообщение классифицировано как реклама.'
    )
    return 'Да'
  }
  ...
  const prompt = `
    ...
    - Призывает подписаться на канал, присоединиться к группе или воспользоваться услугами.
