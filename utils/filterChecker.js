...
export function containsAdContent(text) {
  ...
  const sourcePattern =
    /РИА Новости|РИА|Корреспондент|Источник|Новости|сообщает|по данным|из сообщения|информационное агентство|по словам|по информации/i
  ...
  // Проверка на источники и корреспондентов
  const sourceMatch = text.match(sourcePattern)
  if (sourceMatch) {
    logWithTimestamp(
      `Сообщение заблокировано из-за упоминания источника или корреспондента: "${colors.red}${sourceMatch[0]}${colors.reset}"`,
      'info'
    )
    return true
  }

  // Проверка на благотворительные термины
