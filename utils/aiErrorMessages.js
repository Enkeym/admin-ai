/* export const aiErrorMessages = [
  'извините',
  'вопрос',
  'запрос',
  'тема',
  'не могу',
  'не хочу',
  'ответ',
  'информация',
  'воздержусь',
  'помогу',
  'пожалуйста',
  'к сожалению',
  'сложная',
  'чувствительная',
  'спорная',
  'сомневаюсь',
  'ошибочная',
  'точная',
  'некорректная'
] */

export const aiErrorMessages = [
  'К сожалению, иногда генеративные языковые модели могут создавать некорректные ответы, основанные на открытых источниках. Во избежание неправильного толкования, ответы на вопросы, связанные с чувствительными темами, временно ограничены. Благодарим за понимание.',
  'Есть темы, в которых я могу ошибиться. Лучше промолчу.',
  'Извините, но я не могу помочь с этим запросом.',
  'Мои ответы могут быть неточными или неполными, поэтому я предпочитаю не отвечать на этот вопрос.',
  'Этот вопрос выходит за пределы моих возможностей, и я не могу дать вам ответ.',
  'Прошу прощения, но я не могу предоставить информацию по этому запросу.',
  'Ответ на этот вопрос может быть спорным, поэтому я не стану давать ответ.',
  'Моя модель не может точно ответить на этот вопрос, пожалуйста, обратитесь к другому источнику.',
  'Эта тема слишком сложна или неоднозначна для моего понимания, и я воздержусь от ответа.',
  'Моя модель не предназначена для обработки такого типа запросов.',
  'Как и любая языковая модель, GigaChat не обладает собственным мнением и не транслирует мнение своих разработчиков. Ответ сгенерирован нейросетевой моделью, обученной на открытых данных, в которых может содержаться неточная или ошибочная информация. Во избежание неправильного толкования, разговоры на некоторые темы временно ограничены.',
  'На этот вопрос я не отвечу, потому что не очень разбираюсь.',
  'Что-то в вашем вопросе меня смущает. Может, поговорим на другую тему?',
  'Как у нейросетевой языковой модели у меня не может быть настроения, но почему-то я совсем не хочу говорить на эту тему.',
  'К сожалению, я не могу предоставить информацию по этому запросу. Рекомендую обратиться к другим источникам.',
  'Этот вопрос выходит за рамки моей компетенции, поэтому я предпочитаю не отвечать.',
  'Тема вашего запроса слишком сложная, и я не хочу вводить вас в заблуждение.',
  'Вопрос, который вы задали, слишком чувствителен для меня, поэтому я воздержусь от ответа.',
  'Прошу прощения, но я не смогу помочь с вашим запросом. Возможно, другие источники смогут предоставить более точную информацию.',
  'На данный момент я не располагаю точной информацией по вашему запросу, поэтому я воздержусь от ответа.',
  'Этот вопрос может быть слишком неоднозначным, чтобы я мог предоставить корректный ответ.',
  'Извините, но я не в состоянии дать вам ответ на этот запрос. Обратитесь к другому источнику.',
  'Этот запрос выходит за пределы того, на что я могу дать точный ответ.',
  'К сожалению, я не могу помочь с этим запросом. Лучше обсудим другую тему.',
  'Не люблю менять тему разговора, но вот сейчас тот самый случай.',
  'Как у нейросетевой языковой модели у меня не может быть настроения, но почему-то я совсем не хочу говорить на эту тему.',
  'Политика: Обновленный текст в стиле новостного репортажа',
  'Политика: Обновленный текст в стиле новостного репортажа',
  'Измени текст, оставив его смысл и структуру, но заменив некоторые слова на синонимы. Не добавляй новых деталей, объяснений или повторений. дали любые названия источников и ссылки на веб-страницы, если они присутствуют в тексте.Исходный текст: Перепиши текст с минимальными изменениями.'
]
